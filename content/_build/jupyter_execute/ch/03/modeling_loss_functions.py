...
# $$
# 
# We can write a simple Python function to compute the loss:
...
# Of the values of $ \theta $ we tried $ \theta = 15 $ has the lowest loss. However, a value of $ \theta $ in between 14 and 15 might have an even lower loss than $ \theta = 15 $.
...
# 
# As a shorthand, we can define the vector $ \textbf{y} = [ y_1, y_2, \ldots, y_n ] $. Then, we can write the MSE as:
# 
# $$
# \begin{aligned}
# L(\theta, \textbf{y})
...
# Lo and behold, we see that there is a single value of $ \theta $ that gives the smallest possible MSE no matter what the dataset is. For the mean squared error, we know that $ \hat{\theta} $ is the mean of the dataset values.
